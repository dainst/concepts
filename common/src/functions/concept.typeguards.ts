...
export const isRelatedConcept = (thing: unknown): thing is RelatedConcept =>
  (isConceptAbstract(thing))
	&& ('relations' in thing)
	&& (Array.isArray(thing.relations))
	&& (thing.relations.every(isRelationSet));
...
export const isConcept = (thing: unknown): thing is Concept =>
  (isConceptAbstract(thing))
	...
	&& ((!('relations' in thing)) || ('relations' in thing && Array.isArray(thing.relations) && thing.relations.every(isRelationSet)));
...
export const isTemporalBound = (thing: unknown): thing is TemporalBound =>
  (typeof thing === 'object')
	&& (thing != null)
	&& ('precision' in thing)
	&& ((typeof thing.precision === 'number') || (thing.precision == null))
	&& ('certainty' in thing)
	&& ((typeof thing.certainty === 'number') || (thing.certainty == null))
	&& ('min' in thing)
